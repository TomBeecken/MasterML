...
def expand_words_dimension_mean(
    ...
    for dimension in seed_words:
        ...
        if filter_word_set is not None:
            similar_words = [
                x for x in similar_words if x not in filter_word_set]
        similar_words = [
...
def read_dict_from_csv(file_name):
    ...
    for k in culture_dict.keys():
        culture_dict[k] = set(
            [x for x in culture_dict[k] if x == x])  # remove nan
    ...
    for dim in culture_dict.keys():
        print("Number of words in {} dimension: {}".format(
            dim, len(culture_dict[dim])))
...
def score_tf(documents, document_ids, expanded_words, n_core=1):
    ...
    df = pd.DataFrame(
        results, columns=sorted(
            list(expanded_words.keys())) + ["document_length"]
    )
